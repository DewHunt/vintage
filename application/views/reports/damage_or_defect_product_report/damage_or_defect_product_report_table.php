<div class="card card-boarder">
    ...
    $company_information;
    $currency_settings;
    $damage_or_defect_product_info_by_date_and_branch;
    ...
    <?php if ((strtolower($user_type) != 'marketing')) { ?>
        <?php if (!empty($print_access) > 0) { ?>
            <div class="col-xs-12 col-sm-4">
                <button type="button" class="btn btn-primary damage-or-defect-info-report-print-button report-print-button"><i class="fa fa-print" aria-hidden="true"></i> Print</button>
            </div>
            <?php
        }
    }
    ...
    <table class="table table-striped" style="width: 100%" id="details-table">
        ...
        <tbody>
            ...
            $count = 1;
            $total = 0;
            foreach ($damage_or_defect_product_info_by_date_and_branch as $damage_or_defect_product) {
                ?>
                <?php $damage_or_defect_date = date("d-m-Y", strtotime($damage_or_defect_product->damage_or_defect_date)); ?>
                <?php $total += (double) $damage_or_defect_product->total_purchase_price; ?>
                <tr>
                    <td><?= $count++ ?></td>
                    <td><?= $damage_or_defect_date ?></td>
                    <td><?= $damage_or_defect_product->branch_name ?></td>
                    <td><?= $damage_or_defect_product->total_quantity ?></td>
                    <td><?= number_format((double) $damage_or_defect_product->total_purchase_price, 2) ?></td>
                    <td><?= ucfirst($damage_or_defect_product->reason) ?></td>
                    <td><?= ucfirst($damage_or_defect_product->user_name) ?></td>
                    <td>
                        <button class="btn btn-primary damage-or-defect-product-details-view-button-<?= $damage_or_defect_product->id ?>"
                                data-toggle="tooltip" data-placement="bottom"
                                title="View Details"
                                data-id="<?= $damage_or_defect_product->id ?>"
                                data-action="<?= base_url('reports/damage_or_defect_product_report/damage_or_defect_product_report_details_show_in_modal') ?>">
                            <i class="fa fa-eye" aria-hidden="true"></i></button>
                        <script>
                            $('.damage-or-defect-product-details-view-button-<?= $damage_or_defect_product->id ?>').on('click', function (event) {
                                event.preventDefault();
                                $.post($(this).attr('data-action'), {'id': $(this).attr('data-id')}, function (data) {
                                    $('.damage_or_defect_product_report_modal .damage_or_defect_product_report_modal_show').html(data)
                                    $('.damage_or_defect_product_report_modal').modal('show');
                                });
                            });
                        </script>
                    </td>
    ...
    <div class="modal fade damage_or_defect_product_report_modal">
        <div class="modal-dialog modal-lg damage_or_defect_product_report_modal_show" role="document">
        </div>
    </div>
</div>
...
<div id="damage-or-defect-info-report-print-section" style="width: 100%; display: none;">
...
<!--        <h6 style="text-align: center" class="left-side-view"><?= $company_information->company_address_1 ?></h6>-->
    ...
    <div><strong>Product (Damage / Defect) Report</strong></div>
...
//    });
    ...
    $(".damage-or-defect-info-report-print-button").on("click", function () {

        var divContents = $('#damage-or-defect-info-report-print-section').html();
