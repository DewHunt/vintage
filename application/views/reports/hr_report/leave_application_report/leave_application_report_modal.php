<div class="modal-content">
    ...
    <div class="modal-footer">
        <button type="button" class="btn btn-success leave-application-accept-button"><i class="fa fa-save" aria-hidden="true"></i> Accept</button>
        <label class="btn btn-warning leave-application-reject-button" data-toggle="modal" data-target="#reject-reason-modal">Reject</label>
        <button type="button" class="btn btn-primary leave-application-print-button"><i class="fa fa-print" aria-hidden="true"></i> Print
        </button>
...
<div id="leave-application-modal-print-information" style="display: none">
    <div class="form-group col-xs-12">
...
<script language="javascript" type="text/javascript">
     ...
     $(".leave-application-accept-button").on("click", function () {
        var leave_application_id = $('#leave_application_id').val().trim();
        $.ajax({
            type: "POST",
            url: '<?php echo base_url("reports/hr_report/employee_leave_report/employee_leave_report/accept_leave_application/"); ?>',
            data: {'leave_application_id': leave_application_id},
            success: function (data) {
                if (data == 'yes') {
                    $('.leave-application-details-information-modal').modal('hide');
                } else { //data == 'no'
                    alert('Insufficient Leave');
                }
            },
            error: function () {
    ...
    $(".leave-application-print-button").on("click", function () {

        var divContents = $('#leave-application-modal-print-information').html();
