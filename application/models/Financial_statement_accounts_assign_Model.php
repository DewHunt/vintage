...
class Financial_statement_accounts_assign_Model extends CI_Model {
    ...
    public function get_financial_statement_accounts_assign_by_head_type_and_id($head_type = null, $financial_statement_accounts_id) {
        if ($head_type != null) {
            if ((int) $financial_statement_accounts_id == 4 || (int) $financial_statement_accounts_id == 5) {
                $where_condition = "WHERE h.is_active = '1' AND h.head_type = '$head_type' ORDER BY h.head_name ASC ";
            } else {
                $where_condition = "WHERE h.is_active = '1' AND h.head_type = '$head_type' AND (fa.financial_statement_accounts_id = $financial_statement_accounts_id OR fa.financial_statement_accounts_id IS null) ORDER BY h.head_name ASC";
            }
        } else {
            $where_condition = "WHERE h.is_active = '1' AND (fa.financial_statement_accounts_id = $financial_statement_accounts_id OR fa.financial_statement_accounts_id IS null) ORDER BY h.head_name ASC";
        }
    ...
    public function get_account_statement_report_for_dr($start_date, $end_date, $financial_statement_accounts_id) {
        // 1 == for trading account
        // 2 == profit and loss account
        // 3 == profit and loss appropriation account
        // 4 == balance sheet
        // 5 == trail balance
        $head_details_obj = new Head_details_Model();
        ...
        if (!empty($trading_account_balance_for_dr_list)) {
            ...
            foreach ($trading_account_balance_for_dr_list as $trading_account) {
                ...
                $head_info = $head_details_obj->get_head_details($head_id);
                if(!empty($head_info) && (!boolval($head_info->is_active))){
                    continue;
                }
                $single_head_current_balance = $m_daywise_head_posting->get_single_head_current_balance_by_date($head_id, $start_date, $end_date);
                $arr = array(
                    'head_id' => $head_id,
    ...
    public function get_account_statement_report_for_cr($start_date, $end_date, $financial_statement_accounts_id) {
        // 1 == for trading account
        // 2 == profit and loss account
        // 3 == profit and loss appropriation account
        // 4 == balance sheet
    ...
    public function get_account_statement_report_for_dr_exclude_balance_zero($start_date, $end_date, $financial_statement_accounts_id) {
        $dr = $this->get_account_statement_report_for_dr($start_date, $end_date, $financial_statement_accounts_id);
        $dr_array = array();
        $head_name = '';
        foreach ($dr as $d) {
    ...
    public function get_account_statement_report_for_cr_exclude_balance_zero($start_date, $end_date, $financial_statement_accounts_id) {
        $cr = $this->get_account_statement_report_for_cr($start_date, $end_date, $financial_statement_accounts_id);
        $cr_array = array();
        $head_name = '';
        foreach ($cr as $c) {
    ...
    public function financial_account_statement($start_date, $end_date, $financial_statement_accounts_id) {
        $i = 0;
        $total_dr = 0;
        $total_cr = 0;
        $profit = 0;
        ...
        $grand_total_cr = 0;
        $arr = array();
        $dr = $this->get_account_statement_report_for_dr_exclude_balance_zero($start_date, $end_date, $financial_statement_accounts_id);
        $cr = $this->get_account_statement_report_for_cr_exclude_balance_zero($start_date, $end_date, $financial_statement_accounts_id);
    ...
    public function balance($balance_array, $financial_statement_accounts_id = 0) {
        $total_dr = !empty($balance_array[0]['total_dr']) ? (double) $balance_array[0]['total_dr'] : 0;
        $total_cr = !empty($balance_array[0]['total_cr']) ? (double) $balance_array[0]['total_cr'] : 0;
        $difference = !empty($balance_array[0]['difference']) ? (double) $balance_array[0]['difference'] : 0;
    ...
    public function get_total_dr_cr($account_statement, $balance_array, $financial_statement_accounts_id = 0) {
        if ($financial_statement_accounts_id == 4) {
            $account_statement_profit = $account_statement['loss'];
            $account_statement_loss = $account_statement['profit'];
        } else {
        ...
        if ($financial_statement_accounts_id == 4) {
            if ((double) $account_statement_loss != (double) 0) {
                $total_dr = (double) $account_statement_loss;
                $total_cr = 0;
            } else {
                $total_dr = 0;
                $total_cr = (double) $account_statement_profit;
            }
        } else {
            if ((double) $account_statement_loss != (double) 0) {
                $total_dr = (double) $account_statement_loss;
        ...
        if ($financial_statement_accounts_id == 4) {
            ...
            $balance_array[0]['total_dr'] = (!empty($balance_array[0]['total_dr']) ? (double) $balance_array[0]['total_dr'] : 0) + (double) $total_dr;
            $balance_array[0]['total_cr'] = (!empty($balance_array[0]['total_cr']) ? (double) $balance_array[0]['total_cr'] : 0) + $total_cr;
    ...
    public function get_statement($start_date, $end_date, $financial_statement_accounts_id) {
        // 1 == for trading account
        // 2 == profit and loss account
        // 3 == profit and loss appropriation account
        // 4 == balance sheet
        // 5 == trail balance
        $result_array = array();
        if (!empty($start_date) && !empty($end_date) && !empty($financial_statement_accounts_id)) {
            if ((int) $financial_statement_accounts_id == 1) {
                $result_array = $this->get_trading($start_date, $end_date, $financial_statement_accounts_id);
            } elseif ((int) $financial_statement_accounts_id == 2) {
                $result_array = $this->get_profit_loss($start_date, $end_date, $financial_statement_accounts_id);
            } elseif ((int) $financial_statement_accounts_id == 3) {
                $result_array = $this->get_profit_loss_appropriation($start_date, $end_date, $financial_statement_accounts_id);
            } elseif ((int) $financial_statement_accounts_id == 4) {
                $result_array = $this->get_balance_sheet($start_date, $end_date, $financial_statement_accounts_id);
            }
        }
        return $result_array;
    }

    public function get_trading($start_date, $end_date, $financial_statement_accounts_id) {
        $balance_array = $this->financial_account_statement($start_date, $end_date, $financial_statement_accounts_id);
    ...
    public function get_profit_loss($start_date, $end_date, $financial_statement_accounts_id) {
        $balance_array = $this->financial_account_statement($start_date, $end_date, $financial_statement_accounts_id);
        $trading_account_statement = $this->get_trading($start_date, $end_date, 1); // 1 == for trading account
    ...
    public function get_profit_loss_appropriation($start_date, $end_date, $financial_statement_accounts_id) {
        $balance_array = $this->financial_account_statement($start_date, $end_date, $financial_statement_accounts_id);
        $profit_loss_statement = $this->get_profit_loss($start_date, $end_date, 2); // 2 == profit and loss account
        $balance_array = $this->get_total_dr_cr($profit_loss_statement, $balance_array, $financial_statement_accounts_id);
    ...
    public function get_balance_sheet($start_date, $end_date, $financial_statement_accounts_id) {
        // for balance sheet swap dr and cr
        $balance_array = $this->financial_account_statement($start_date, $end_date, $financial_statement_accounts_id);
        $profit_loss_appropriation_statement = $this->get_profit_loss_appropriation($start_date, $end_date, 3); // 3 == profit and loss appropriation account
...
//        echo '</pre>';
    ...
    public function get_trading_and_profit_loss_assign_head_list() {
        $query_result = $this->db->query("SELECT h.id, h.head_name, h.head_type, fa.head_id, fa.financial_statement_accounts_type, fa.financial_statement_accounts_id, f.account_name FROM head_details h LEFT JOIN financial_statement_accounts_assign fa ON fa.head_id = h.id LEFT JOIN financial_statement_accounts f ON fa.financial_statement_accounts_id = f.id WHERE (h.head_type = 'dr' OR h.head_type = 'cr') AND ((fa.financial_statement_accounts_id = 1 OR fa.financial_statement_accounts_id = 2) OR fa.financial_statement_accounts_id IS null) AND h.is_active = '1' ORDER BY h.head_name ASC")->result();
        return $query_result;
    }
