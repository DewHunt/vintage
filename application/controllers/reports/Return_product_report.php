...
class Return_product_report extends CI_Controller {
    ...
    public function index() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('sales_report_access')) == TRUE)) {
            ...
            $this->load->view('reports/return_product_report/return_product_report', $this->data);
        } else {
            redirect(base_url('user_login'));
        }
    }

    public function return_product_report_show_in_table() {  // show in table
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('sales_report_access')) == TRUE)) {
            $user_info = $this->session->userdata('user_session');
            $user_id = $user_info['user_id'];
            $user_type = $user_info['user_type'];
            ...
            if (empty($start_date) || empty($end_date)) {  // please select value= -1
                ...
                $return_product_info_by_date_and_branch = $this->Return_product_info_Model->get_return_product_info_by_date_and_branch($start_date, $end_date, $branch_id);
                $this->data['return_product_info_by_date_and_branch'] = $return_product_info_by_date_and_branch;
                $this->data['branch_name'] = $branch_name;
                $this->data['start_date'] = $start_date;
                $this->data['end_date'] = $end_date;
                $this->load->view('reports/return_product_report/return_product_report_table', $this->data);
            }
        } else {
            redirect(base_url('user_login'));
        }
    ...
    public function return_product_report_details_show_in_modal() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('sales_report_access')) == TRUE)) {
            ...
            $employee_id = $user_info->employee_id;
            $return_product_info_with_to_branch = $this->Return_product_info_Model->get_return_product_info_with_to_branch($id);
            $this->data['return_product_info_with_to_branch'] = $return_product_info_with_to_branch;
            $return_product_details_by_return_product_info_id = $this->Return_product_details_Model->get_return_product_details_by_return_product_info_id($id);
            $this->data['return_product_details_by_return_product_info_id'] = $return_product_details_by_return_product_info_id;
            $company_information = $this->Company_Model->get_company();
            $currency_settings = $this->Currency_settings_Model->get_currency_settings();
            $this->data['company_information'] = $company_information;
            $this->data['currency_settings'] = $currency_settings;
            $this->load->view('reports/return_product_report/return_product_report_modal', $this->data);
        } else {
            redirect(base_url('user_login'));
        }
    }
