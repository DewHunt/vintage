...
class Head_transaction_report extends CI_Controller {
    ...
    public function index() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('accounts_report_access')) == true)) {
            ...
            $this->load->view('reports/accounts_report/head_transaction_report/head_transaction_report', $this->data);
        } else {
            redirect(base_url('user_login'));
        }
    }

    public function Head_transaction_report_show_in_table() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('accounts_report_access')) == true)) {
            ...
            $this->load->view('reports/accounts_report/head_transaction_report/head_transaction_report_table', $this->data);
        } else {
            redirect(base_url('user_login'));
        }
    }
    ...
    public function get_head_details_report_by_head_id($head_id, $start_date, $end_date) {
        $result = $this->db->query("SELECT vpd.id, vpd.posting_date, vpd.voucher_number, vpd.total_debit_amount, vpd.total_credit_amount, vpd.common_narration, vpd.user_id, vd.id AS voucher_details_id, vd.income_head_id, vd.expense_head_id, vd.amount ,vd.invoice_number, vd.mr_number, vd.client_id, vd.employee_id, vd.month, vd.year, vd.narration, vd.debit_amount, vd.credit_amount, vd.opening_balance, vd.closing_balance FROM voucher_posting_details vpd JOIN voucher_details vd ON vpd.id= vd.voucher_posting_details_id WHERE (vd.income_head_id = '$head_id' OR vd.expense_head_id='$head_id') AND vpd.posting_date >= '$start_date' AND vpd.posting_date <= '$end_date' ORDER BY vd.id ASC")->result();
        return $result;
    }
