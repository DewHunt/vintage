...
class Trail_balance_report extends CI_Controller {
    ...
    public function trail_balance_report_show_in_table() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('accounts_report_access')) == TRUE)) {
            ...
            if (empty($year)) {
                ...
                $dr = $this->get_all_dr_or_both_head_current_balance($year);
                $cr = $this->get_all_cr_or_both_head_current_balance($year);
    ...
    public function get_all_dr_or_both_head_current_balance($year) {
        $result_array = array();
        ...
        if (!empty($head_list)) {
            foreach ($head_list as $head_info) {
                ...
                $result = $m_daywise_head_posting->get_current_balance_from_daywise_head_posting_by_head($head_id, $year);
                if (empty($result)) {
                    $result = $m_daywise_head_posting->get_current_balance_from_previous_year_daywise_head_posting_by_head($head_id, $year);
                }
                if (!empty($result)) {
    ...
    public function get_all_cr_or_both_head_current_balance($year) {
        $result_array = array();
        $head_remove_from_all_account_statement = $this->Financial_statement_accounts_assign_Model->head_remove_from_all_account_statement();
        $m_daywise_head_posting = new Daywise_head_posting_Model();
        $m_head_details = new Head_details_Model();
...
//        Closing Inventory(Lubzone-Mohakhali S/R)-142
        $head_id_array = array();
        $head_id_array = array(123, 125, 127, 128, 141, 142);
