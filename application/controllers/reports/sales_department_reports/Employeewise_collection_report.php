...
class Employeewise_collection_report extends CI_Controller {
    ...
    public function index() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('sales_report_access')) == TRUE)) {
            $this->data['title'] = "Employee-wise Collection Report";
            $this->data['page_title'] = "Employee-wise Collection Report";
            ...
            $this->load->view('reports/sales_department_reports/employeewise_collection_report/employeewise_collection_report', $this->data);
        } else {
            redirect(base_url('user_login'));
        }
    }

    public function employeewise_collection_report_show_in_table() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('sales_report_access')) == TRUE)) {
            $this->data['report_title'] = "Employee-wise Collection Report";
...
//                get_print_r($this->data['outstanding_report']);
//                get_print_r($this->db->last_query());
                $this->data['employee_names'] = $employee_names;
                $this->data['start_date'] = $start_date;
                $this->data['end_date'] = $end_date;
                $this->load->view('reports/sales_department_reports/employeewise_collection_report/employeewise_collection_report_table', $this->data);
            }
        } else {
            redirect(base_url('user_login'));
        }
