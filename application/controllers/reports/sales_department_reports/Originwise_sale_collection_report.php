...
class Originwise_sale_collection_report extends CI_Controller {
    ...
    public function index() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('sales_report_access')) == TRUE)) {
            $this->data['title'] = "Origin-wise Sale Collection Report";
            $this->data['page_title'] = "Origin-wise Sale Collection Report";
            ...
            $this->load->view('reports/sales_department_reports/originwise_sale_collection_report/originwise_sale_collection_report', $this->data);
        } else {
            redirect(base_url('user_login'));
        }
    }

    public function originwise_sale_collection_report_show_in_table() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('sales_report_access')) == TRUE)) {
            $this->data['report_title'] = "Origin-wise Sale Collection Report";
            ...
            if (empty($start_month_year) || empty($end_month_year)) {
                ...
                $this->data['originwise_sale_collection_report'] = $this->Employee_Model->get_originwise_sale_collection_report($start_month_year, $end_month_year, $employee_ids);
//                get_print_r($this->data['originwise_sale_collection_report']);
                ...
                $this->load->view('reports/sales_department_reports/originwise_sale_collection_report/originwise_sale_collection_report_table', $this->data);
            }
        } else {
            redirect(base_url('user_login'));
        }
