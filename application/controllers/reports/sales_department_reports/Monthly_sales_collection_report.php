...
class Monthly_sales_collection_report extends CI_Controller {
    ...
    public function index() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('sales_report_access')) == TRUE)) {
            $this->data['title'] = "Monthly Sales Collection Report";
            $this->data['page_title'] = "Monthly Sales Collection Report";
            ...
            $this->load->view('reports/sales_department_reports/monthly_sales_collection_report/monthly_sales_collection_report', $this->data);
        } else {
            redirect(base_url('user_login'));
        }
    }

    public function monthly_sales_collection_report_show_in_table() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('sales_report_access')) == TRUE)) {
            $this->data['report_title'] = "Monthly Sales Collection Report";
            ...
            if (empty($start_date) || empty($end_date)) {
                ...
                $this->data['company_information'] = $this->Company_Model->get_company();
                $this->data['currency_settings'] = $this->Currency_settings_Model->get_currency_settings();
                $this->data['monthly_sales_collection_report'] = $this->Employee_Model->get_monthly_sales_collection_report($start_date, $end_date, $employee_ids);
                $this->data['employee_names'] = $employee_names;
                $this->data['start_date'] = $start_date;
                $this->data['end_date'] = $end_date;
                $this->load->view('reports/sales_department_reports/monthly_sales_collection_report/monthly_sales_collection_report_table', $this->data);
            }
        } else {
            redirect(base_url('user_login'));
        }
