...
class Sales_commission_report extends CI_Controller {
    ...
    public function index() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->is_loggedin_user_type_admin()) == true)) {
            $this->data['title'] = "Sales Commission Report";
            $this->data['page_title'] = "Sales Commission Report";
            ...
            $this->load->view('reports/sales_statement_report/sales_commission_report/sales_commission_report', $this->data);
        } else {
            redirect(base_url('user_login'));
        }
    }

    public function sales_commission_report_show() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->is_loggedin_user_type_admin()) == true)) {
            if ($this->input->is_ajax_request()) {
                $this->data['report_title'] = "Sales Commission Report";
                ...
                if (!empty($start_date) && !empty($end_date) && !empty($client_id != '')) {
                    ...
                    $this->data['sales_commission_report'] = $this->Client_sales_commission_Model->get_sales_commission_report($start_date, $end_date, $client_id, $employee_id);
//                    get_print_r($this->db->last_query());
                    $this->load->view('reports/sales_statement_report/sales_commission_report/sales_commission_report_table', $this->data);
                } else {
                    echo '<div class="error-message text-align-center">Please check input fields.</div>';
                }
            } else {
                redirect(base_url('reports/sales_statement_report/sales_commission_report'));
            }
        } else {
            redirect(base_url('user_login'));
        }
