...
class Employeewise_sales_analysis_report extends CI_Controller {
    ...
    public function index() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->is_loggedin_user_type_admin()) == true)) {
            $this->data['title'] = "Employeewise Sales Analysis";
            $this->data['page_title'] = "Employeewise Sales Analysis";
            ...
            $start_limit = 0;
            $end_limit = 25;
            $employeewise_sales_analysis = $this->Invoice_details_Model->get_employeewise_sales_analysis($start_date, $end_date, $start_limit, $end_limit);
            $this->data['start_date'] = $start_date;
            $this->data['end_date'] = $end_date;
            $this->data['employeewise_sales_analysis'] = !empty($employeewise_sales_analysis) ? json_encode($employeewise_sales_analysis) : '';
            $this->data['user_type'] = $this->User_Model->get_loggedin_user_type();
            $this->data['report_content'] = $this->load->view('reports/sales_statement_report/sales_analysis_report/employeewise_sales_analysis_report/employeewise_sales_analysis', $this->data, TRUE);
            ...
            if ($this->input->is_ajax_request()) {
                ...
                $end_limit = intval(trim($this->input->post('end_limit')));
                 if (!empty($start_date) && !empty($end_date) && ($start_limit >= 0) && ($end_limit > 0)) {
                    $employeewise_sales_analysis = $this->Invoice_details_Model->get_employeewise_sales_analysis($start_date, $end_date, $start_limit, $end_limit);
                    $this->data['employeewise_sales_analysis'] = !empty($employeewise_sales_analysis) ? json_encode($employeewise_sales_analysis) : '';
                    $html = $this->load->view('reports/sales_statement_report/sales_analysis_report/employeewise_sales_analysis_report/employeewise_sales_analysis_bar_chart', $this->data, TRUE);
                    set_json_output(array('html' => $html));
                } else {
                    $message = 'Please Check Input Fields.';
                }
            } else {
                redirect(base_url('reports/sales_statement_report/sales_analysis_report/employeewise_sales_analysis_report'));
            }
        } else {
            redirect(base_url('user_login'));
        }
