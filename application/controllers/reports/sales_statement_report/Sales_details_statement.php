...
class Sales_details_statement extends CI_Controller {
    ...
    public function index() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->is_loggedin_user_type_admin()) == true)) {
            $this->data['title'] = "Sales Details Statement";
            $this->data['page_title'] = "Sales Details Statement Report";
            ...
            $this->load->view('reports/sales_statement_report/sales_details_statement/sales_details_statement', $this->data);
        } else {
            redirect(base_url('user_login'));
        }
    }

    public function sales_details_statement_report_show() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->is_loggedin_user_type_admin()) == true)) {
            if ($this->input->is_ajax_request()) {
                $this->data['report_title'] = "Sales Details Statement Report";
                ...
                if (!empty($start_date) && !empty($end_date) && ($client_id != '')) {
                    ...
                    $employee_id = $this->User_Model->get_loggedin_user_employee_id();
                    $this->data['sales_details_statement_report'] = $this->Invoice_details_Model->get_sales_details_statemnt_report($start_date, $end_date, $client_id, $user_type, $employee_id);
                    $this->load->view('reports/sales_statement_report/sales_details_statement/sales_details_statement_table', $this->data);
                } else {
