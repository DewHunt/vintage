...
class Employee_salary_generate extends CI_Controller {
    ...
    public function index() {
        if (get_user_permission('employee_salary_generate') === false) {
        ...
        $this->data['salary_information_details_list_html'] = $this->load->view('employee/salary_information_details_list', $this->data, TRUE);
...
//            $this->data['year_session'] = $year_session;
        ...
        $previous_date_time = date('Y-m-t', strtotime($previous_date_time)); // last date t
        if ($select_month_year <= $previous_date_time) {
            $this->salary_details_save($employee_list, $select_month_year, $month, $year, $user_id);
            $message = 'Employee Salary has been generated Successfully.';
            $is_generate = TRUE;
        } else {
            $message = 'Please Select Correct Month and Year.';
            $is_generate = FALSE;
        }
        $this->data['employee_salary_details'] = $this->get_employee_salary_details_by_employee_id_month(0, $month, $year); //get list
        $this->data['month'] = $month;
        $this->data['year'] = $year;
        $this->data['is_preview'] = $is_preview;
        $html = $this->load->view('employee/salary_information_details_list', $this->data, TRUE);
        $data_array = array('message' => $message, 'isGenerate' => $is_generate, 'htmlView' => $html);
    ...
    public function get_date_by_month_name($month, $year) {
        ...
        $select_month_year = get_start_date_format(date('Y-m-t', strtotime($select_month_year)));
    ...
    public function salary_details_save($employee_list, $select_month_year, $month, $year, $user_id) {
        ...
        if (!empty($employee_list)) {
            foreach ($employee_list as $employee) {
                if (intval($employee->id) == 6) { // admin employee id = 6
                    continue;
                }
...
//                        redirect(base_url('employee_salary_generate'));
    ...
    public function salary_details_preview() {
        if (get_user_permission('employee_salary_generate') === false) {
            redirect(base_url('user_login'));
        }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $is_preview = TRUE;
            $salary_array = array();
            $this->data['title'] = "Employee Salary Preview";
            $user_info = $this->session->userdata('user_session');
            $user_id = $user_info['user_id']; // session user id
            $month = trim($this->input->post('month'));
            $year = trim($this->input->post('year'));
...
//                if ($select_month_year <= $previous_date_time) {
            ...
            if (!empty($employee_list)) {
                foreach ($employee_list as $employee) {
                    ...
                    if ($employee_id == 6) { // admin employee id = 6
                        continue;
                    }
                    if (((int) $employee->deactivate_employee) <= 0) {
                        $joining_date = date('Y-m-d', strtotime($employee->joining_date));
                        ...
                        if ((($joining_date <= $select_month_year))) {
                            if (($closing_date >= $select_month_year) || (!empty(($employee->closing_date == "0000-00-00 00:00:00")))) {
                                $salary_details_by_month_year_employee_id = $this->Salary_details_Model->get_salary_details_by_month_year_employee_id($employee_id, $month, $year);
                                $is_preview = !empty($salary_details_by_month_year_employee_id) ? FALSE : TRUE;
                                $salary_details_id = !empty($salary_details_by_month_year_employee_id) ? intval($salary_details_by_month_year_employee_id->id) : 0;
                                $current_date_time = !empty($salary_details_by_month_year_employee_id) ? ($salary_details_by_month_year_employee_id->current_date_time) : get_current_date_and_time();
                                /* edit */
                                $loan_information = $this->Loan_Model->get_loan($employee->current_loan_id);
                                if (!empty($loan_information)) {
                                    $per_installment_amount = $loan_information->per_installment_amount;
                                ...
                                $take_home_salary = $net_salary - $per_installment_amount;
                                $pf_and_loan = ((double) $employee->pf_contribution) + ((double) $per_installment_amount);
                                $employee_benefit_amount = $this->get_employee_benefit_total_by_month_year($month, $year, $employee_id);
                                $data = array(
                                    'id' => $salary_details_id,
                                    'employee_id' => $employee_id,
                                    ...
                                    'is_loan' => $employee->is_loan,
                                    'pf_contribution_company_part' => $employee->pf_contribution_company_part,
                                    'basic_salary' => $employee->basic_salary,
                                    'phone_allowance' => $employee->phone_allowance,
                                    'tuition_allowance' => $employee->tuition_allowance,
                                    'attendance_allowance' => $employee->attendance_allowance,
                                ...
                                array_push($salary_array, $data_obj);
                            }
                        }
                    }
                }
                $this->data['employee_salary_details'] = $salary_array;
                $this->data['month'] = $month;
                $this->data['year'] = $year;
                $this->data['is_preview'] = $is_preview;
                $html = $this->load->view('employee/salary_information_details_list', $this->data, TRUE);
...
//                    $message = 'Please Select Correct Month and Year.';
//                }
            $data_array = array('message' => $message, 'htmlView' => $html);
            set_json_output($data_array);
        } else {
            redirect(base_url('employee_salary_generate'));
        }
