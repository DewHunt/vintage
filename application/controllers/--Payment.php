...
class Payment extends CI_Controller {
    ...
    public function save_payment() {
        if (!empty($this->session->userdata('user_session')) && (($this->User_Model->get_user_permission('sales_access')) == true)) {
            ...
            $branch_mr_no = trim($this->input->post('branch_mr_no'));
            ...
            if (!empty($valid_numeric_number_check_result) == TRUE) {
                $data = array(
                    ...
                    'bank_id' => $bank_id,
                    'branch_name' => !empty($bank_information) ? $bank_information->branch_name : '',
                    'purpose' => trim($this->input->post('purpose')),
                    'invoice_number' => trim($this->input->post('invoice_number')),
                    'remarks' => $remarks,
                    'branch_id' => $branch_id,
                    'branch_mr_no' => $branch_mr_no
                );
                $this->form_validation->set_rules('client_id', 'Client', 'required');
                $this->form_validation->set_rules('amount_received', 'Amount', 'required');
